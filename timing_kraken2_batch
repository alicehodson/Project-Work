...
def reads_output(file_name, size):
    with open(file_name, "r") as fh:
       subprocess.Popen(f'time kraken2 --db /home/mbyah11/kraken2-microbial-fatfree/ --use-names --threads 4 {file_name} --output batch_{size}_run_{sample_letter}.output'.split()).communicate()
